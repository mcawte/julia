...
mutable struct TTY <: LibuvStream
    ...
    lock::ReentrantLock
    throttle::Int
    @static if iswindows(); ispty::Bool; end
...
const OS_HANDLE = iswindows() ? WindowsRawSocket : RawFD
const INVALID_OS_HANDLE = iswindows() ? WindowsRawSocket(Ptr{Void}(-1)) : RawFD(-1)
