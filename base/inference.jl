...
function isinlineable(m::Method, src::CodeInfo)
    ...
    if !inlineable
        inlineable = inline_worthy_stmts(src.code, cost_threshold)
    end
    return inlineable
end
...
inline_worthy(body::ANY, cost_threshold::Integer=200) = true

## Computing the cost of a function body
# saturating sum (inputs are nonnegative), prevents overflow with typemax(Int) below
plus_saturate(x, y) = max(x, y, x+y)

statement_cost(::Any, line::Int) = 0
statement_cost(mi::MethodInstance, line::Int) =
    (isleaftype(mi.specTypes) & isleaftype(mi.rettype)) ? 0 : 100
statement_cost(qn::QuoteNode, line::Int) = statement_cost(qn.value, line)
function statement_cost(gn::GotoNode, line::Int)
...
function statement_cost(ex::Expr, line::Int)
    head = ex.head
    ...
    argcost = 0
    for a in ex.args
        argcost = plus_saturate(argcost, statement_cost(a, line))
    end
    if head == :return || head == :(=)
        return argcost
    end
            ...
            if isa(grfunc, Const)
                ...
                if isa(f, IntrinsicFunction)
                    ...
                    if !isassigned(t_ifunc_cost, iidx)
                        ...
                        return plus_saturate(argcost, 100)
                    end
                ...
                if isa(f, Function)
                    ...
                    if f == Main.Core.getfield || f == Main.Core.tuple
                        return plus_saturate(argcost, isleaftype(ex.typ) ? 1 : 100)
                    elseif f == Main.Core.arrayref
                        return plus_saturate(argcost, isleaftype(ex.typ) ? 4 : 100)
                    end
                    fidx = findfirst(t_ffunc_key, f::Function)
                    if fidx == 0
                        # unknown/unhandled builtin or anonymous function
...
function inline_worthy_stmts(stmts::Vector{Any}, cost_threshold::Integer=200)
    body = Expr(:block)
    body.args = stmts
    return inline_worthy(body, cost_threshold)
end

function inline_worthy(body::Expr, cost_threshold::Integer=200)
    bodycost = 0
    if body.head == :block
        for line = 1:length(body.args)
            stmt = body.args[line]
            thiscost = statement_cost(stmt, line)
            bodycost = plus_saturate(bodycost, thiscost)
        end
    else
        bodycost = statement_cost(body, 1)
    end
