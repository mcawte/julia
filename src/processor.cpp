...
struct TargetArg {
    ...
    FeatureList<n> enable;
    FeatureList<n> disable;
};

template<typename CPU, size_t n, typename F>
static inline std::vector<TargetArg<CPU,n>> parse_cmdline(const char *option,
                                                          const CPUSpec<CPU,n> *cpus,
                                                          size_t ncpu, F &&cb, uint32_t def=0)
{
    std::vector<TargetArg<CPU,n>> res;
    TargetArg<CPU,n> arg;
    auto reset_arg = [&] {
        arg.cpu = static_cast<CPU>(def);
        arg.clone_all = false;
        arg.name.clear();
        arg.extra_features.clear();
        memset(&arg.enable[0], 0, 4 * n);
...
constexpr size_t ncpu_names = sizeof(cpus) / sizeof(cpus[0]);
...
{
    using namespace X86;
    return jl_cstr_to_string(find_cpu_name(host_cpu.first, cpus, ncpu_names));
}
...
jl_sysimg_fptrs_t jl_init_processor_sysimg(void *hdl)
{
    ...
    return parse_sysimg(hdl, [] (void *id) {
            auto targets = deserialize_target_data(cpus, ncpu_names, (const uint8_t *)id);
            // TODO
            return 0;
        });
}
