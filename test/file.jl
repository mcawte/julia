...
if !iswindows() || Sys.windows_version() >= Sys.WINDOWS_VISTA_VER
    ...
    @test readlink(dirlink) == subdir * (iswindows() ? "\\" : "")
end
...
cd(dirwalk) do
    ...
    has_symlinks = !iswindows() || (Sys.windows_version() >= Sys.WINDOWS_VISTA_VER)
