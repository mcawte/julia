...
io = (text) -> begin
    write(filename, text)
    open(`$(iswindows() ? "type" : "cat") $filename`)[1]
...
@test write(io, "abc") == 3
close(io)
if !iswindows() && get(ENV, "USER", "") != "root" && get(ENV, "HOME", "") != "/root"
    ...
    iswindows() || warn("file permissions tests skipped due to running tests as root (not recommended)")
    close(open(f))
end
chmod(f, 0o400)
f1 = open(f)
